...
def mean_normalized_error_power(predictions, ground_truth):
    ...
    for pred_meter, ground_truth_meter in both_sets_of_meters:
        ...
        # TODO: if sum of ground truth is zero, change it for a tiny float to avoid
        # zero division errors.
        if sum_of_ground_truth_power == 0:
            sum_of_ground_truth_power = 1e-100
...
def tp_fp_fn_tn(predictions, ground_truth):
    both_sets_of_meters = iterate_through_submeters_of_two_metergroups(
        predictions, ground_truth)
    ...
    for pred_meter, ground_truth_meter in both_sets_of_meters:
        ...
        for aligned_states_chunk in aligned_meters:
            ...
            tp_sum += np.sum(np.logical_and(
                aligned_states_chunk['master'] == True,
                aligned_states_chunk['slave'] == True
            ), axis=0)
            fp_sum += np.sum(np.logical_and(
                aligned_states_chunk['master'] == True,
                aligned_states_chunk['slave'] == False
            ), axis=0)
            fn_sum += np.sum(np.logical_and(
                aligned_states_chunk['master'] == False,
                aligned_states_chunk['slave'] == True
            ), axis=0)
            tn_sum += np.sum(np.logical_and(
                aligned_states_chunk['master'] == False,
                aligned_states_chunk['slave'] == False
            ), axis=0)

        instances.append(pred_meter.instance())
        tps.append(tp_sum)
        fps.append(fp_sum)
        fns.append(fn_sum)
        tns.append(tn_sum)
